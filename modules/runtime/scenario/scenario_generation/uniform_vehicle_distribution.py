...
class UniformVehicleDistribution(ScenarioGeneration):
  ...
  def initialize_params(self, params):
    params_temp = self._params["Scenario"]["Generation"]["UniformVehicleDistribution"]

    self._map_file_name = params_temp["MapFilename", "Path to the open drive map", 
                  "modules/runtime/tests/data/Crossing8Course.xodr"]
    self._ego_goal = params_temp["EgoGoal", "The center of the ego agent's goal region polygon", [-191.789,-50.1725] ]
    self._others_source = params_temp["OthersSource", "A list of points around which other vehicles spawn. \
    ...
    self._others_sink = params_temp["OthersSink", "A list of points around which other vehicles are deleted. \
    ...
    self._vehicle_distance_range = params_temp["VehicleDistanceRange", "Distance range between vehicles in meter given as tuple from which distances are sampled uniformly", (40, 50)]
    self._velocity_range = params_temp["VehicleVelocityRange", "Lower and upper bound of velocity in km/h given as tuple from which velocities are sampled uniformly", (20,30)]
  ...
  def create_single_scenario(self):
    ...
    for idx, source in enumerate(self._others_source):
      connecting_center_line, s_start, s_end, _, lane_id_end = \
                self.center_line_between_source_and_sink( world.map,  source, self._others_sink[idx])
      ...
      agent_list.extend( self.place_agents_along_linestring(world, connecting_center_line, s_start, s_end, \
                                                                        self._agent_params, goal_definition) )
    ...
    goal_polygon = goal_polygon.translate(Point2d(self._ego_goal[0], self._ego_goal[1]))
