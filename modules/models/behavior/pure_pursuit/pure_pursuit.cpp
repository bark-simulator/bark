...
float BehaviorPurePursuit::FindSteeringAngle(const dynamic::State &agent_state, const geometry::Line &line) const {
  ...
  // TODO(@AKreutz): get gain and wheelbase from params
  float gain = 1.0f;
  float Lf = 2.6f;
  ...
  float lookahead_distance = gain * agent_state(StateDefinition::VEL_POSITION);
  float nearest_s = get_nearest_s(line, agent_position);
  Point2d lookahead_point = get_point_at_s(line, nearest_s + lookahead_distance);

  float lookahead_angle = atan2(bg::get<1>(lookahead_point - agent_position), bg::get<0>(lookahead_point - agent_position)) - agent_state(StateDefinition::THETA_POSITION);
  
  return atan(2*Lf*sin(lookahead_angle) / distance(agent_position, lookahead_point));
}
