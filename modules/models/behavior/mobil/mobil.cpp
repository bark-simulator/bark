...
Trajectory BehaviorMobil::Plan(float delta_time,
  ...
  if (agent_in_front.first) {
    net_distance = CalcNetDistanceFromFrenet(
        ego_agent, frenet_ego, agent_in_front.first, agent_in_front.second + frenet_ego);
  ...
  for (int i = 1; i < num_traj_time_points; ++i) {
    ...
    traj(i,StateDefinition::VEL_POSITION) = std::max(traj(i,StateDefinition::VEL_POSITION), 0.0f);
  }
...
BehaviorMobil::CheckIfLaneChangeBeneficial(
  ...
  if (leader_current.first) {
    const double dist_c_before = CalcNetDistanceFromFrenet(
        ego_agent, frenet_ego, leader_current.first, leader_current.second + frenet_ego);
    acc_c_before =
  ...
  if (follower_current.first) {  // there is a follower
    ...
    const double dist_o_before = CalcNetDistanceFromFrenet(
        follower_current.first, follower_current.second + frenet_ego, ego_agent, frenet_ego);
    ...
    if (leader_current.first) {
      const double dist_o_after = CalcNetDistanceFromFrenet(
          follower_current.first, follower_current.second + frenet_ego, leader_current.first,
  ...
  if (right_corridor) {
    ...
    if (leader_right.first) {
      ...
      const double dist_c_after = CalcNetDistanceFromFrenet(
          ego_agent, frenet_ego, leader_right.first, leader_right.second + frenet_ego);
      acc_c_after =
    ...
    if (follower_right.first) {
      ...
      if (leader_right.first) {
        const double dist_n_before = CalcNetDistanceFromFrenet(
            follower_right.first, follower_right.second + frenet_ego, leader_right.first,
      ...
      const double dist_n_after = CalcNetDistanceFromFrenet(
          follower_right.first, follower_right.second + frenet_ego, ego_agent, frenet_ego);
  ...
  if (left_corridor) {
    ...
    if (leader_left.first) {
      ...
      // ---------------------------------------------------
      const double dist_c_after = CalcNetDistanceFromFrenet(
          ego_agent, frenet_ego, leader_left.first, leader_left.second + frenet_ego);
        acc_c_after =
            CalcRawIDMAcc(dist_c_after, vel_ego,
    ...
    if (follower_left.first) {
      ...
      if (leader_left.first) {
        const double dist_n_before =
            CalcNetDistanceFromFrenet(follower_left.first, follower_left.second + frenet_ego,
      ...
      // ---------------------------------------------------
      const double dist_n_after = CalcNetDistanceFromFrenet(
          follower_left.first, follower_left.second + frenet_ego, ego_agent, frenet_ego);
    ...
    if (acc_n_after >= -safe_deceleration_) {
      ...
      if (benefit > threshold) {
        if (asymmetric_passing_rules_) {
          ...
          VLOG(2) << "Go left. Ego improves from " << acc_c_before << " to" << acc_c_after << ", others improve by " <<
           acc_n_after - acc_n_before;
        }
        acc_threshold = benefit;
        return std::make_pair(LaneChangeDecision::ChangeLeft, left_corridor);
      } else {
        VLOG(2) << " Benefit " << acc_c_after << " - " << acc_c_before << " + " << politeness_ << " * (" << acc_n_after << " - " << acc_n_before << ") = " << benefit
