...
BehaviorMobil::CheckIfLaneChangeBeneficial(
  ...
  if (leader_current.first) {  // there is a leader
    ...
    if (right_corridor) {
      ...
      std::pair<AgentPtr, FrenetPosition> follower_right =
          agents_right_lane.rear;
      std::pair<AgentPtr, FrenetPosition> leader_right =
          agents_right_lane.front;
      ...
      if (acc_n_after >= -safe_deceleration_) {
        ...
        if (asymmetric_passing_rules_) {
          ...
          benefit = acc_c_after - acc_c_before +
                    politeness_ * (acc_n_after - acc_n_before + acc_o_after -
                                   acc_o_before);
        }
        ...
        if (benefit > threshold) {
          ...
          return std::make_pair(LaneChangeDecision::ChangeRight,
                                right_corridor);
        }
      } else {
