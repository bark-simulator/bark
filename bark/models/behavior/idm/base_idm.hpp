...
public:
  ...
  virtual std::tuple<Trajectory, Action>
  GenerateTrajectory(const world::ObservedWorld &observed_world,
  ...
  GetTotalAcc(const world::ObservedWorld &observed_world,
              const IDMRelativeValues &rel_values, double rel_distance,
              double dt) const;

  IDMRelativeValues
  CalcRelativeValues(const world::ObservedWorld &observed_world,
