...
TEST(behavior_static_trajectory_plan, calculate_action) {
  ...
  0, 0, -1.75, 0, 1,
  1, 1, -1.75, 0, 1;
  auto action = BehaviorStaticTrajectory::CalculateAction(1.0, observed_world, static_traj1);
  ...
  action = BehaviorStaticTrajectory::CalculateAction(2.0, observed_world, static_traj2);
  ...
  1, 1, -1.75, 3.14/2.0, 1,
  2, 1, -3,    3.14/2.0, 1;
  action = BehaviorStaticTrajectory::CalculateAction(2.0, observed_world, static_traj3);
  lon_lat_action = boost::get<LonLatAction>(action);
  EXPECT_NEAR(lon_lat_action.acc_lon, 0.0, 0.001);
  ...
  1, 1, -1.75, -3.14/2.0, 1,
  2, 1, +0, -3.14/2.0, 1;
  action = BehaviorStaticTrajectory::CalculateAction(2.0, observed_world, static_traj4);
  lon_lat_action = boost::get<LonLatAction>(action);
  EXPECT_NEAR(lon_lat_action.acc_lon, 0.0, 0.001);
  EXPECT_NEAR(lon_lat_action.acc_lat, -1/2.0, 0.001);
  ...
  1, 1, -1.75, 3.14/4.0, 1,
  2, 1, +0, 3.14/4.0, 3;
  action = BehaviorStaticTrajectory::CalculateAction(2.0, observed_world, static_traj5);
  lon_lat_action = boost::get<LonLatAction>(action);
